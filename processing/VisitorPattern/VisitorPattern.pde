// https://forum.processing.org/one/topic/octree-visualization-toxiclibs.html
...
// recursive function applying a visitor to all nodes in the tree
void applyVisitor(PointOctree node, OctreeVisitor visitor) {
  visitor.visitNode(node);
  if (node.getNumChildren()>0) {
    for (PointOctree c : node.getChildren()) {
      if (c!=null) {
        applyVisitor(c, visitor);
      }
    }
  }
}