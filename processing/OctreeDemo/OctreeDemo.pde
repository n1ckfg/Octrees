// https://openprocessing.org/sketch/9669#
...
void draw() {
  ...
  if (useSphere) {
    points=(ArrayList<Vec3D>) octree.getPointsWithinSphere(cursor,clipRadius);
  } 
  else {
    points=(ArrayList<Vec3D>) octree.getPointsWithinBox(new AABB(cursor,clipRadius));
  }
